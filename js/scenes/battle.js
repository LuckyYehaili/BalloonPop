...
module.exports = {
  ...
  _startPump() {
    ...
    this._pumpTimer = setInterval(() => {
      ...
      const jitter = (Math.random() - 0.5) * instability * 1.5; // 波动项
      const increment = Math.max(0.15, base + jitter); // 最小 0.15，防止卡死
      const next = Math.min(state.pressure + increment, 100);
      state.pressure = next;
      if (next >= 100) {
